...
export class Topics extends Component {
  ...
  renderTopics () {
    let topics = this.props.topics.map(topic => (<li><Topic edit={this.state.edit} topic={topic} tallies={this.props.talliesForCurrentDate[topic.id] || 0} /></li>))
    return (<ul>{topics}</ul>)
  }

  renderEditButton () {
    let copy = this.state.edit ? 'Finished' : 'Manage Tallies'
